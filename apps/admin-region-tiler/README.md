...
一个自托管的 Go Web 地图瓦片下载工具，支持范围框选、行政区划/GeoJSON、
多地图源、任务进度、失败记录和 ZIP/MBTiles 产物下载。
...
1. 下载源代码后在对应平台编译，或使用项目根目录 README 中的 Docker 启动方式。

2. GitHub Release 发布后，可从发布页面下载对应平台的预编译程序。
...
登录默认启用，开发默认账号为 `admin` / `adminmap`。可信本地环境可设置
`AUTH_ENABLED=false` 免登录；生产部署应保持登录启用并通过 `.env` 覆盖默认账号。
